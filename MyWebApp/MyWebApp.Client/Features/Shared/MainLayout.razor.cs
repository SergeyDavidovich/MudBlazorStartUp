...
namespace MyWebApp.Client.Features.Shared
{
    public class MainLayoutBase : LayoutComponentBase
    {
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            // Обратите внимание: это не работает должным образом, если включен предварительный рендеринг на сервере. Так как макет
            // отображается с использованием любой темы по умолчанию (или конкретной темы) сервера, затем, когда
            // клиентская часть готова, она запускается, определяет предпочтения локальной системы пользователя и применяется
            // желаемая тема. Однако, если системные настройки отличаются от исходных, предварительно обработанных
            // html, то тема применяется после задержки, что приводит к изменению темы после небольшой задержки. Там
            // Кажется, нет никакого способа обойти это, пока не станет возможным получить системные предпочтения, чтобы
            // тема предварительного рендеринга совпадает с фактическими системными настройками клиента!
